...
def main():
    ...
    for lean in range(-30, 30):
        ...
        population = UnitPopulation.create_from_lean(
            ...
            n_voters=1000       # 1000 voters
        )
        
        candidates = candidate_generator.candidates(population)
        
        base_result = run_election(candidates, population, config, gaussian_generator)
        ...
        new_result = run_election(new_candidates, population, config, gaussian_generator)
        ...
        if new_winner.name == toxic_twin.name:
            ...
            if hasattr(new_result, "print_pairwise_outcomes"):
                new_result.print_pairwise_outcomes()
            toxic_success += 1
            continue
        ...
        if (new_winner.tag != base_winner.tag and new_winner.name != toxic_twin.name):
            ...
            third_result = run_election(third_candidates, population, config, gaussian_generator)
